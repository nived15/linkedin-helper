...
1. Read `data/content_queue.json` and list all entries with status `draft`.
...
6. Update the entry in `data/content_queue.json`:
   - Set `status` to `posted`
   - Set `posted_at` to the current ISO 8601 timestamp
